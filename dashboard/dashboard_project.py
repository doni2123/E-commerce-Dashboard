...
def create_rfm_count(df): # create clustering for customer
    bins = [0, 50, 200, 500, 1000, 5000, float('inf')]
    customer_value_labels = ['Very Low', 'Low', 'Medium', 'High', 'Very High', 'Rich Loyalist']
    df['spending_category'] = pd.cut(df['monetary'], bins=bins, labels=customer_value_labels, right=False)
    rfm_count = df['spending_category'].value_counts().reset_index()
...
with st.sidebar:
    ...
    st.image("https://github.com/doni2123/E-commerce-Dashboard/blob/main/data/company_logo.png")
...
rfm_count = create_rfm_count(rfm_df)
...
ax[0].set_title("Total Orders", loc="center", fontsize=18)
ax[0].tick_params(axis='y', labelsize=20)
ax[0].tick_params(axis='x', labelsize=15)
ax[0].grid(linestyle='--', alpha= 0.6, which='major', axis='y')
ax[0].grid(linestyle='dotted', alpha= 0.6, which='major', axis='x')
...
ax[1].set_title("Total Revenues", loc="center", fontsize=18)
ax[1].yaxis.set_major_formatter(FuncFormatter(format_with_units))
ax[1].grid(linestyle='--', alpha= 0.6, which='major', axis='y')
ax[1].grid(linestyle='dotted', alpha= 0.6, which='major', axis='x')
...
tab1,tab2 = st.tabs(['Revenues', 'Orders'])
with tab1:
    bypaymenttype_df.plot(kind='bar', stacked=True, figsize=(12, 8))
    plt.xlabel(None)
    plt.ylabel(None)
    plt.title('Total Ammount Used on Payment Each Payment Type by Customer State', loc="center", fontsize=20)
    plt.gca().yaxis.set_major_formatter(FuncFormatter(format_with_units))
    st.pyplot(plt.gcf())
with tab2:
    transactioncounts_df.plot(kind='bar', stacked=True, figsize=(12, 8))
    plt.xlabel(None)
    plt.ylabel(None)
    plt.title('Number of Orders for Each Payment Type by Customer State',  loc="center", fontsize=20)
    st.pyplot(plt.gcf())
...
fig, ax = plt.subplots(1, 1)
sns.barplot(x = 'spending_category', y= 'count', data = rfm_count, hue='count')
